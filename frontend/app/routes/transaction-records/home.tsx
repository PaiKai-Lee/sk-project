...
export default function TransactionRecordsHome() {
    ...
    const specificTransactionQuery = useQuery({
        ...
        queryFn: async () => {
            console.log('trigger')
            const { data } = await TransactionsClient.getOneTransaction(transactionId as string);
            return data;
        },
    });
        ...
        () => [
            ...
            },
            {
                ...
                cell: (info) => <>
                    ...
                    <Text className='text-xs text-muted-foreground'>{info.row.original.createdByUser.uid}</Text>
                </>,
            },
            {
            ...
            },
            {
                ...
                cell: (info) => <Button size={'sm'} className='cursor-pointer' data-transaction-id={info.row.original.transactionId} onClick={clickDetailHandler}>Details</Button>,
            }
        ],
        []
    );
    ...
    return (
        <>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                {specificTransactionQuery.isLoading && <Skeleton className='h-[300px] w-full' />}
                    ...
                    <DialogContent className='md:max-w-2xl lg:max-w-3xl'>
                        <DialogHeader>
                            ...
                            <DialogDescription className='flex flex-col gap-1 lg:flex-row '>
                                <Text className='md:basis-1/2'><strong>Transaction ID:</strong> {specificTransactionQuery.data.data?.transactionId}</Text>
                                <Text className='md:basis-1/2'><strong>Remark:</strong> {specificTransactionQuery.data.data?.remark}</Text>
                            </DialogDescription>
                        </DialogHeader>
                        <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                                {specificTransactionQuery.data.data?.transactionsItems.map((item) => (
                                    <TableRow key={item.id} className={`${item.value > 0 ? 'bg-green-100' : 'bg-red-100'}`} >
                                        ...
                                        <TableCell className="font-medium">{dateFormatter.format(new Date(item.createdAt))}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
